...
namespace FellrnrTrainingAnalysis.Model
{
    [MemoryPackable]
    [Serializable]
    public partial class TimeSeriesCalculateSpeed : TimeSeriesEphemeral
    {
        [MemoryPackConstructor]
        protected TimeSeriesCalculateSpeed()  //for use by memory pack deserialization only
        {
            SportsToInclude = new List<string>(); //keep the compiler happy
        }
        public TimeSeriesCalculateSpeed(string name, Activity parent, bool persistCache, List<string>? requiredFields, List<string>? opposingFields = null, List<string>? sportsToInclude = null) :
        ...
        public override TimeValueList? CalculateData(int forceCount, bool forceJustMe)
        {
            ...
            float pace = distance / elapsed; //meters/second

            float[] speeds = new float[seconds]; //tempting to add this as a data stream, but probably should be a new computation
            for (uint i = 0; i < seconds; i++)
            {
                speeds[i] = pace;
            }

            TimeValueList retval = new TimeValueList(speeds);
